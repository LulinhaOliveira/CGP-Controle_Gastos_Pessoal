...
class UserController {
    ...
    async login(request, response){
        ...
        if(!user || !bcryptjs.compareSync(password,user.password)){
            return response.status(404).send({Messagem: "Email e/ou Senha Invalidos"})
        }
        ...
        const { id } = request.params;
            ...
            try{
                ...
                if(user){
                    ...
                    return response.status(404).send({Messagem: "Usuario Não Encontrado"})
                }
    ...
    async store(request, response){
        ...
        const schema = yup.object().shape({
            email : yup.string("O e-mail deve ser String").required("O e-mail é obrigatório.").email("O e-mail precisa ser válido."),
            password : yup.string("A senha deve ser String").required("A senha é obrigatório").min(5,"Deve ter ao menos Cinco caracteres"),
            nome : yup.string("O nome deve ser uma String").required("O nome é obrigatório"),
            })

            await schema.validate({email, password, nome})
            .then(async () => {
    ...
    async uptadeAll(request, response){
        ...
        const schema = yup.object().shape({
            password : yup.string("A senha deve ser String").min(5,"Deve ter ao menos Cinco caracteres"),
            nome : yup.string("O nome deve ser uma String").min(4)
            })
      
            await schema.validate({nome, password})
            .then( async ()=>{
    ...
    async updateSaldo_Mensal(request, response){
        ...
        const schema = yup.object().shape({
            saldo_mensal: yup.number().required("Digite um valor").positive("O valor deve ser maior que 0").typeError("Deve ser um numero")
            })

      
            await schema.validate({saldo_mensal})
            .then( async ()=>{
                await prismaClient.user.update({where: {id}, data: {saldo_mensal}})
                .then(()=> response.status(200).send({Messagem: "Saldo Atualizado"}))
                .catch((err)=> response.status(400).send({Error: err}));
            })
            .catch((err)=>response.status(400).send({Error: err.errors}))
    ...
    async updateDate_Rece(request,response){
        ...
        if(dat_recebe){
            ...
            await prismaClient.user.update({where: {id}, data: {dat_recebe: dat_nasc_parse}})
