...
const ControlAcessMiddleware = (request, response, next) =>{
    ...
    if(request.url === '/cgp/login' || (request.url === '/cgp/users' && request.method === 'POST')
     ...
     || (request.url === 'cpg/cartoes' && request.method === 'POST')){
        return next();
    }
    ...
    if(paramRota  === "categorias"){
        ...
        if(paramId !== undefined){
            (async () => {
                ...
                }).catch((err) => {return response.status(400).send({Messagem : err})})
            ...
            (async () =>{
                await prismaClient.categorias.findUnique({
                    ...
                    await prismaClient.cartao.findUnique({
                        ...
                        if(results.id_user === loggedUser.id && id_categoria_user === loggedUser.id){
                            return next();
                        }else{
                            return response.status(401).send({Messagem : "Esse dado não é seu"});
                        }
